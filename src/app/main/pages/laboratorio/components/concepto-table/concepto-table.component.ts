  ...
  construirTabla(){
    ...
    eliminarConceptosSelected() {
        ...
        for (let i = 0; i < this.selectedConceptos.length; i++) {
            this.conceptosService.deleteObject(this.selectedConceptos[i].idConcepto).subscribe(
                data => {
                    indexLista++;
    ...
    async eliminarConceptoSimple(doc: ConceptoDto) {
        this.conceptosService.deleteObject(doc.idConcepto).subscribe(
            data => {
    ...
    exportPdf() {
        ...
        this.appservie.exportPdf(this.exportColumns, this.listConceptos, 'Conceptos', "p");
    }
